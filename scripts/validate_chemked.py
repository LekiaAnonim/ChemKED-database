#!/usr/bin/env python
"""
Dispatch PyKED schema validation for contributed ChemKED files.

Fires a ``pyked-validate`` repository_dispatch to kineticmodelssite.
The self-hosted runner there runs ci_validate_chemked.py (which has PyKED
installed) and posts results back to this PR as a check-run.
No PyKED/pyyaml install needed here.
...
def dispatch_pyked_validation(files, pr_repo, pr_number, commit_sha,
    ...
    body = json.dumps({"event_type": "pyked-validate", "client_payload": payload}).encode()
    ...
    print(f"Dispatched pyked-validate to {pyteck_owner}/{pyteck_repo} "
...
def main():
    ...
    if not files:
        print("No YAML files to dispatch for PyKED validation.")
        return

    dispatch_token = os.environ.get("DISPATCH_TOKEN", "")
    if not dispatch_token:
        print("::warning::DISPATCH_TOKEN not set — skipping PyKED dispatch.")
        return
    ...
    if not pr_repo or not commit_sha:
        print("::warning::PR_REPO or PR_HEAD_SHA not set — skipping PyKED dispatch.")
        return
    ...
    dispatch_pyked_validation(files, pr_repo, pr_number, commit_sha,
                              dispatch_token, pyteck_owner, pyteck_repo)
